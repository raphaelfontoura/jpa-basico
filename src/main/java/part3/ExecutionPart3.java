...
public class ExecutionPart3 {
    ...
    public static void main(String[] args) {
        ...
        // // Trazendo uma lista de alunos de um estado específico
        // String sqlListEstado = "SELECT * FROM Aluno WHERE estado_id = :idEstado";
        // List<Aluno> alunosSqlListEstado = entityManager
        //         .createNativeQuery(sqlListEstado, Aluno.class)
        //         .setParameter("idEstado", 1)
        //         .getResultList();

        // alunosSqlListEstado.forEach(System.out::println);
        ...
        // Trazendo uma lista de alunos pelo estado
        String jpqlListAlunoEstado = "select a from Aluno a where a.estado.sigla = :estadoSigla";
        List<Aluno> alunoJpqlListEstado = entityManager
                .createQuery(jpqlListAlunoEstado, Aluno.class)
                .setParameter("estadoSigla", "RJ")
                .getResultList();

        alunoJpqlListEstado.forEach(System.out::println);
        ...
        // CriteriaBuilder.In<String> inClause = entityManager.getCriteriaBuilder().in(alunoRoot.get(Aluno_.nome)); //metamodel Aluno_
        // inClause.value(nome);
